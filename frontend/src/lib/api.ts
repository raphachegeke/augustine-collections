...
export const ordersApi = {
  ...
  getMy: (token: string) =>
    request<any[]>("/orders/my", { headers: authHeaders(token) }),
  getAll: (token: string) =>
    request<any[]>("/orders", { headers: authHeaders(token) }),
};
...
export const mpesaApi = {
  ...
  manualConfirm: (
    token: string,
    body: { orderId: string; receiptNumber: string },
  ) =>
    request<any>("/mpesa/manual-confirm", {
      method: "POST",
      headers: authHeaders(token),
      body: JSON.stringify(body),
    }),
