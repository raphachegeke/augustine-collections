  ...
  return (
    <Layout>
      <div className="container py-12">
        <h1 className="text-3xl font-bold mb-2 text-blue-600">Shop by Category</h1>
        <p className="text-muted-foreground mb-8">Browse clothing categories at Augustine's Collections</p>
        {loading && <LoadingSpinner />}
        {error && <p className="text-red-500">{error}</p>}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {categories.map((cat, i) => (
            <motion.div key={cat._id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
              <Link to={`/categories/${cat._id}`} className="group block bg-white border border-blue-100 rounded-lg overflow-hidden hover:shadow-lg transition-shadow">
